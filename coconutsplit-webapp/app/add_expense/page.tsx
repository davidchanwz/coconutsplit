...
export default function AddExpense() {
  ...
  useEffect(() => {
    if (splitMode === 'equal' && amount && members.length > 0) {
      ...
      if (!isNaN(amountValue)) {
        ...
        const equalAmount = (amountValue / members.length).toFixed(2);
        
        // Update splits object
        const newSplits: { [key: string]: string } = {};
        members.forEach(member => {
          newSplits[member.uuid] = equalAmount;
        });
        setSplits(newSplits);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Math.abs(splitsTotal - amountValue) > 0.01) {
      setError(`Split total (${splitsTotal.toFixed(2)}) doesn't match expense amount (${amountValue.toFixed(2)})`);
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="p-3 bg-gray-800 border border-gray-700 rounded text-white">
