...
│   ├── grouphandlers.py     # Group-related message handlers
│   ├── expensehandlers.py   # Expense-related message handlers
│
...
### Expenses Splits Table

Stores information about the expenses splits between each user  in each group.
...
| ------------ | --------- | ------------------------------------ |
| `expense_id` | UUID    | Unique identifier for the expense    |
| `user_id`    | UUID    | UUID of the user who split              |
| `amount`     | DECIMAL   | Total amount of the split          |
---

### Debts Table

Stores debts between group members.
...
- **`add_debt(user: User, amount_owed: float)`**:  
  Adds an entry to the `debts` table, representing how much a user owes for this specific expense. If a debt already exists between the user and the payer for the given group, the method updates the existing record instead of creating a new one.
...
- **`add_split(user: User, amount: float)`**:  
  Adds split to the `expenses_splits` table, representing how much a user owes for this specific expense.

- **`fetch_splits()`**:  
  Retrieves all splits associated with the given expense and returns a list of split data.
---